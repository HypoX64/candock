...
#                     result[i][j] = dsp.signa123l2spectrum(data[i][j],opt.stft_size,opt.stft_stride,
...
def ToInputShape(opt,data,test_flag = False):
    ...
    if opt.mode in ['classify_1d','domain_1d']:
        result = augmenter.ch1d(opt, data, test_flag = test_flag).astype(np.float32)

    elif opt.mode in ['classify_2d','domain','dml']:
        h,w = opt.img_shape
        result = np.zeros((opt.input_nc,h,w), dtype=np.float32)
        data = augmenter.ch1d(opt, data, test_flag = test_flag)

        for i in range(opt.input_nc):
            result[i] = dsp.signal2spectrum(data[i],opt.stft_size,opt.stft_stride,
                opt.cwt_wavename,opt.cwt_scale_num,opt.spectrum_n_downsample,not opt.stft_no_log, mod=opt.spectrum)
